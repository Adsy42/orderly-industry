...
async def isaacus_classify(
    ...
    try:
        ...
        query = supabase_client.from_("document_embeddings").select(
            ...
            try:
                ...
                for classification in classifications:
                    if classification.get("score", 0) > 0.6:  # Confidence threshold
                        classified_clauses.append(ClassifiedClause(
                            clause_type=classification["label"],
                            confidence=classification["score"],
                            text_excerpt=chunk_text[:500],  # Truncate for response size
